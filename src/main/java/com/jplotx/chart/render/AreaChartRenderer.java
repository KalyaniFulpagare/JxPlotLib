...
public final class AreaChartRenderer implements ChartRenderer<XYDataset> {
    ...
    public void render(Graphics2D g2, PlotContext context, ChartSpec spec, XYDataset dataset, PlotOptions options) {
        ...
        for (XYSeries series : seriesList) {
            ...
            XYPoint first = series.points().get(0);
            double firstX = area.x + mapX(0, first, dataset, xScale, categories.size(), area.width);
            polygon.moveTo(firstX, baseline);
            for (int i = 0; i < series.points().size(); i++) {
                XYPoint point = series.points().get(i);
                double x = area.x + mapX(i, point, dataset, xScale, categories.size(), area.width);
                double y = area.y + area.height - yScale.map(point.y(), area.height);
                polygon.lineTo(x, y);
            }
            XYPoint last = series.points().get(series.points().size() - 1);
            double lastX = area.x + mapX(series.points().size() - 1, last, dataset, xScale, categories.size(), area.width);
            polygon.lineTo(lastX, baseline);
            polygon.closePath();

            Color fill = new Color(series.color().getRed(), series.color().getGreen(), series.color().getBlue(), 90);
            g2.setColor(fill);
            g2.fill(polygon);
            ...
            for (int i = 0; i < series.points().size() - 1; i++) {
                ...
                double ax = area.x + mapX(i, a, dataset, xScale, categories.size(), area.width);
                double ay = area.y + area.height - yScale.map(a.y(), area.height);
                double bx = area.x + mapX(i + 1, b, dataset, xScale, categories.size(), area.width);
    ...
    private double mapX(int index, XYPoint point, XYDataset dataset, NumericScale xScale, int categoryCount, int width) {
        if (dataset.useNumericX()) {
            return xScale.map(point.x(), width);
        }
        return width * GraphicsSupport.categoryFraction(index, categoryCount, false);
    }
